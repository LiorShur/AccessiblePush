...
async function handleReviewAction(action, id) {
  if (action === 'approve') {
    ...
    try {
      // Flip the route: reviewed, approved, and public so it shows up
      // in user-facing browse queries (which filter by isPublic).
      ...
      // Also flip any linked trail_guide docs. If the surveyor's save
      // didn't generate a guide, this loop is a no-op and users see
      // the summary via the admin's Inspect modal only (until a guide
      // is generated by a subsequent Publish step).
